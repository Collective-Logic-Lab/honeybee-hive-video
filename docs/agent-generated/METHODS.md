...
## HV-P001: Packaged download and resequencing tools

Status: adopted for issue #6 implementation, 2026-09-06.

Decision: `hive_video.download` and `hive_video.resequence` hold the reusable implementations alongside `hive_video.fragment`. The command groups are `fragment`, `download`, and `resequence`; cluster callers invoke the installed modules directly. The unreleased interface has no legacy forwarding scripts or compatibility aliases. Resequencing exposes its existing stages individually; source-cut inspection, automatic join QC, and hash-bound manual approval remain explicit steps. The grouped `resequence render` command applies the existing QC and approval checks before calling the renderer, binding its source and complete order to the report; lower-level renderer callers retain responsibility for those checks. Installation supplies library and command-line access without bundling internal analysis, experimental recipes, or scheduler infrastructure. Resequencing dependencies are an optional installation extra.

Rationale and evidence: the downloader's archive selection/TLS path and the resequencer's deterministic helpers have focused tests, and isolated package installations reproduce tiny local artifacts. The Slurm workers hash the package implementation files in their restart signatures, including the detector used by join QC. Equivalence checks compare the same tiny inputs and existing test expectations before and after relocation; operational code locations and resulting signatures may change, while scientific parameters and output schemas remain fixed.

Alternatives and sensitivity: compatibility forwarders and alternate command spellings are unnecessary before the first public API or release. Copying a second implementation would allow methods to diverge; an unattended combined resequencing command would conceal required review boundaries. Code relocation deliberately changes current restart signatures; prior artifacts and launched plans remain associated with their recorded revisions. No jobs, transfers, or new experiments are authorized by package installation.

Affected scope: future calls through this package revision and its cluster workers. Existing method decisions HV-R001 through HV-R004 remain in force. Revisit the stage interface only with explicit evidence that review and provenance requirements remain enforceable; define compatibility requirements when a public interface is released.

API archive defaults, 2026-09-06: at the user's request, `resolve_video` defaults to the CLI's Edmond server and dataset DOI, and `download_video` defaults to the same server. Explicit overrides remain available; an alternate server must be passed to both calls. Recording selection, cache and output paths, and transfer settings remain explicit. This makes ordinary API examples use the package's established archive without repeating its address; existing calls and scheduled workers supplying those settings retain their behavior. Revisit these defaults if the package's source archive changes.

Direct download API, 2026-09-06: the user's requested `download_video(day=22, side=0, panel="top", target=...)` supersedes the preceding requirement for explicit cache paths and transfer settings. `day` selects the archive's sequential `start<N>` identifier; it does not denote elapsed days or establish calendar chronology. The filename timestamp remains authoritative. Selection and destination remain explicit; manifest caching defaults to the user cache directory, keyed by server and DOI. Transfer defaults match the CLI: a 120-second timeout, eight attempts, progress reports every 60 seconds when a callback is supplied, MD5 verification enabled, and forced downloading disabled. A previously resolved `RemoteFile` remains accepted for inspecting the published identity and size before transfer; combining it with direct selection or resolution-only overrides is rejected. This keeps the existing transfer and checksum procedure while removing an unnecessary resolution step from ordinary calls. Existing runs and scheduled worker settings are unaffected. Revisit the `day` terminology if a calendar-date selector is introduced.

## HV-P003: Local automatic resequencing with one join-review boundary

Status: adopted for the requested notebook simplification, 2026-09-09. Supersedes HV-P001's exclusion of a combined interface; existing stages and launched Slurm plans retain their behavior.

Decision: expose `run_resequence` / `hive-video resequence run` with an explicitly selected, versioned `edmond-2019-v1` profile. It follows the unattended path in HV-R002: detect, summarize, use the unchanged proposed cuts, build segments, order, and run HV-R001 join QC. An `auto_pass` proceeds through the existing guarded renderer. A `manual_review_required` outcome writes the complete flagged-join roll and stops without a final video. After watching every flagged join, one explicit `approve_resequence` / `resequence finish` call records the reviewer's identity and note with the existing artifact-bound approval, then applies the guarded renderer. Unacceptable joins still require correction through the staged tools.

The profile retains the Slurm detector cap of 400 candidates, width 128 and robust cutoff 12; adjacent candidates are grouped with gap 1 and only single-jump events are proposed. Ordering uses 10-frame trajectories at width 96 with top-k 10. QC retains maximum robust z 15 and minimum runner-up margin 2. Review clips show two seconds per side at width 824 with green join markers; the final viewing render uses width 824 and source-frame captions, followed by the pilot's `low` H.264 sharing derivative (CRF 28, medium preset, one encoder thread) for notebook playback. This profile requires 25 fps input. Source frame counts are explicitly decoded for segment construction. All stage arguments are recorded in the local run manifest. The renderer's existing nominal terminal-frame omission remains unchanged.

Rationale and evidence: the scheduler supplies orchestration, not a different reconstruction algorithm. Moving that orchestration into the package makes the notebook short while retaining inspectable intermediate artifacts and the existing gate. Source cuts remain labeled `unreviewed_pilot`, including after join approval; join QC cannot detect a missed cut inside a segment or prove absolute chronology. The bundled raw example previously produced a flagged join with robust z about 54.45, so an unattended invocation must stop for review on that input rather than relax thresholds.

Alternatives and sensitivity: retaining the full staged notebook requires users to manage implementation details; automatic approval would erase the review boundary. Fresh output directories, bound QC artifacts, recorded package/source identity, and explicit approval preserve that boundary. Failures remain failures; no automatic retry, overwrite, upload, or scheduling is added. Local results are viewing derivatives with local provenance, not newly validated inventory. Revisit the profile when source-cut or join-QC methods change, or labeled failures justify recalibration. Validate this wrapper against the existing stages on a deterministic passing fixture and the supplied flagged sample before using it in the notebook.

## HV-P002: Managed FFmpeg executables

Status: adopted for the unreleased package, 2026-09-06.

Decision: install `portable-ffmpeg==0.3.0` and `filelock` as core Python dependencies to provision both FFmpeg and ffprobe. An explicit pair of absolute `HIVE_VIDEO_FFMPEG` and `HIVE_VIDEO_FFPROBE` paths takes precedence; otherwise use a complete pair on `PATH`, or download and cache the provider's platform-specific version-8 pair on first media use. `hive-video setup-ffmpeg` prepares the pair before offline use and reports executable paths, actual versions, and SHA256 checksums. Help and archive downloading do not provision video executables. A process lock serializes provisioning and fails after 60 seconds of contention; it never breaks another process's lock. Failed selection, download, or execution remains a failure; an invalid explicit pair never falls back to another build.

Rationale and evidence: Python wrappers do not necessarily supply either executable. A candidate wheel labeled macOS universal2 contained only Intel executables. Another provider, `static-ffmpeg==3.0`, supplied native Apple Silicon binaries reporting 7.0 despite its 8.0 description; the unchanged fragment command produced only 24 decodable frames for a requested 25, independently confirmed with both managed and system decoders. It was rejected. The selected provider's native Apple Silicon 8.0 pair passes the 23 existing deterministic fragment tests and produces the requested 25 frames from the repository seed. Frame selection, encoding parameters, and validation were unchanged for that comparison.

The selected provider offers macOS and Linux ARM64/x86-64 and Windows x86-64 downloads. Its cache belongs to the installed environment. Provision it before parallel or scheduled work; Sol workers continue to select their existing module and pass its resolved executable paths explicitly, so a compute task does not download tools. Unsupported native architectures fail without substitution. Windows and Linux support here reflects provider configuration, not execution testing on those operating systems.

Alternatives and sensitivity: requiring a system package manager adds a separate installation task; maintaining platform wheels here adds a binary distribution workflow. The version-8 selector uses 8.0 on macOS and a rolling 8.1 release asset on Windows/Linux. The Python dependency pin does not freeze downloaded bytes; the provider does not verify upstream archive checksums. Setup records the actual executable identity after download, and fragment sidecars retain the actual versions and commands. Existing scientific runs remain bound to their recorded environments and revisions; exact reproduction requires those builds. Provisioning changes do not alter QC thresholds or establish equivalence of arbitrary binary versions.

Affected scope: future package installations and executable resolution in fragment and resequencing calls. Review binary provenance, platform support, and redistribution obligations as part of the separately authorized release work. Revisit if provider availability, cache handling, codec support, or an offline distribution requirement changes.
